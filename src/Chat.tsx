    ...
    async (question: string) => {
      ...
      if (videoId === '') {
        ...
        requestAPI<any>('segments', {
          ...
          .catch(reason => {
            console.error(
              `Error on POST /intelligent-tutor/chats .\n${reason}`
            );
          });
      } else {
        setIsTyping(true);
        ...
        if (question !== '') {
          requestAPI<any>('update_bkt', {
            ...
            .catch(reason => {
              console.error(
                `Error on POST /intelligent-tutor/update_bkt .\n${reason}`
              );
            });
        }
        ...
        if (category === 'Self-exploration') {
          ...
          if (canGoOn === false) {
            requestAPI<any>('go_on', {
              ...
              .catch(reason => {
                console.error(
                  `Error on POST /intelligent-tutor/go_on .\n${reason}`
                );
              });
          }
        }
  ...
  const handleGoOn = () => {
    ...
    if (currentSegmentIndex < segments.length - 1) {
      requestAPI<any>('go_on', {
        ...
        .catch(reason => {
          console.error(`Error on POST /intelligent-tutor/go_on .\n${reason}`);
        });
      ...
      requestAPI<any>('update_seq', {
        ...
        .catch(reason => {
          console.error(
            `Error on POST /intelligent-tutor/update_seq .\n${reason}`
          );
        });
