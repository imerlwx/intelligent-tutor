...
class SegmentHandler(APIHandler):
    ...
    def get(self):
        self.finish(
            json.dumps({"data": "This is /intelligent-tutor/segments endpoint!"})
        )
    ...
    def post(self):
        ...
        if notebook:
            ...
            if CUR_SEQ != [] and question == "":
                ...
                if "step-index" in parameters:
                    input_data["step_index"] = current_step_index + 1
...
def initialze_database():
    ...
    segments_set = [
        ...
        {"category": "Interpret the chart", "start": 900, "end": 968},  # 11
        {"category": "Visualize the data", "start": 968, "end": 1027},  # 12
    ]
    segments_json = json.dumps(segments_set)
    c.execute("SELECT * FROM segments_cache WHERE video_id = ?", (video_id,))
    if c.fetchone() is None:
...
def setup_handlers(web_app):
    ...
    data_pattern = url_path_join(base_url, "intelligent-tutor", "data")
    handlers = [(data_pattern, DataHandler)]
    web_app.add_handlers(host_pattern, handlers)

    # Add route for getting csv data
    update_seq_pattern = url_path_join(base_url, "intelligent-tutor", "update_seq")
    handlers = [(update_seq_pattern, UpdateSeqHandler)]
    web_app.add_handlers(host_pattern, handlers)

    # Add route for getting video segments
    segment_pattern = url_path_join(base_url, "intelligent-tutor", "segments")
    handlers = [(segment_pattern, SegmentHandler)]
    web_app.add_handlers(host_pattern, handlers)

    # Add route for getting code file
    code_pattern = url_path_join(base_url, "intelligent-tutor", "code")
    handlers = [(code_pattern, CodeHandler)]
    web_app.add_handlers(host_pattern, handlers)

    # Add route for getting chat response
    chat_pattern = url_path_join(base_url, "intelligent-tutor", "chat")
    handlers = [(chat_pattern, ChatHandler)]
    web_app.add_handlers(host_pattern, handlers)

    # Add route for getting go on or not response
    go_on_pattern = url_path_join(base_url, "intelligent-tutor", "go_on")
    handlers = [(go_on_pattern, GoOnHandler)]
    web_app.add_handlers(host_pattern, handlers)

    # Add route for getting go on or not response
    update_bkt_pattern = url_path_join(base_url, "intelligent-tutor", "update_bkt")
    handlers = [(update_bkt_pattern, UpdateBKTHandler)]
    web_app.add_handlers(host_pattern, handlers)

    # Add route for getting go on or not response
    fill_blank_pattern = url_path_join(base_url, "intelligent-tutor", "fill_blank")
